...
def scene_from_url(url: str) -> ScrapedScene | None:
    ...
    # Scrape movies from "Found in these movies:" section
    if movies_header := soup.find("div", string="Found in these movies:"):
        movies_container = movies_header.parent
        if movies_container:
            movie_links = movies_container.find_all("a", href=lambda x: x and "video/" in x)
            if movie_links:
                scene["movies"] = [  # type: ignore
                    name_with_url(movie_link) for movie_link in movie_links
                ]
    
    # Scrape movies from "Found in this movie:" section
    if movies_header := soup.find("div", string="Found in this movie:"):
        movies_container = movies_header.parent
        if movies_container:
            movie_links = movies_container.find_all("a", href=lambda x: x and "video/" in x)
            if movie_links:
...
def movie_from_url(url: str) -> ScrapedMovie | None:
    ...
    if covers := movie_section.find_all("img", src=lambda x: "Covers" in x):
        movie["front_image"] = abs_url(covers[0]["image"])
        if len(covers) > 1:
            movie["back_image"] = abs_url(covers[1]["image"])
