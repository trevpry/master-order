...
if [ "$DATABASE_URL" = "file:/app/data/master_order.db" ] || echo "$DATABASE_URL" | grep -q "file:"; then
    ...
    export DATABASE_URL="postgresql://master_order_user:${POSTGRES_PASSWORD:-secure_password_change_me}@192.168.1.122:5432/master_order"
...
echo "[DEBUG] Testing network connectivity to 192.168.1.122:5432..."
if ping -c 1 192.168.1.122 >/dev/null 2>&1; then
    echo "[SUCCESS] Can ping 192.168.1.122"
else
    echo "[ERROR] Cannot ping 192.168.1.122"
fi

echo "[DEBUG] Testing if port 5432 is open on 192.168.1.122..."
if nc -z 192.168.1.122 5432 2>/dev/null; then
    echo "[SUCCESS] Port 5432 is open and accepting connections on 192.168.1.122"
else
    echo "[ERROR] Port 5432 is not accessible on 192.168.1.122"
...
nslookup 192.168.1.122 2>/dev/null || echo "[INFO] nslookup failed"
...
if command -v psql >/dev/null 2>&1; then
    ...
    PGPASSWORD=secure_password_change_me timeout 10 psql -h 192.168.1.122 -p 5432 -U master_order_user -d master_order -c "SELECT current_database(), current_user, version();" 2>&1
else
