...
if timeout 5 bash -c "echo >/dev/tcp/192.168.1.114/5432" 2>/dev/null; then
...
timeout 5 bash -c "exec 3<>/dev/tcp/192.168.1.114/5432; echo 'Connected' >&3; cat <&3" 2>/dev/null && echo "[SUCCESS] Raw TCP connection works" || echo "[ERROR] Raw TCP connection failed"
