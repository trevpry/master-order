...
router.get('/cache/health', async (req, res) => {
  try {
    ...
    console.error('Error checking artwork cache health:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to check artwork cache health',
...
router.post('/cache/repair', async (req, res) => {
  try {
    ...
    console.error('Error during artwork cache repair:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to repair artwork cache',
      details: error.message
    });
  }
});
...
router.get('/:filename', async (req, res) => {
  try {
    ...
    res.status(500).send('Failed to serve artwork');
  }
});
