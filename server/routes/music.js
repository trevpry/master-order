...
router.delete('/custom-playlists/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
router.post('/custom-playlists/:id/tracks', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to add track to custom playlist' });
  }
});

// Remove Track from Custom Playlist
router.delete('/custom-playlists/:id/tracks/:trackId', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to remove track from custom playlist' });
  }
});
...
router.get('/artists', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch music artists' });
  }
});

// Music Artists - By Section
router.get('/artists/section/:sectionKey', async (req, res) => {
  try {
    ...
    console.error('Error fetching music artists by section:', error);
    res.status(500).json({ error: 'Failed to fetch music artists by section' });
  }
});

// Music Artists - Single Artist
router.get('/artists/:ratingKey', async (req, res) => {
  try {
...
router.get('/albums/section/:sectionKey', async (req, res) => {
  try {
    const { sectionKey } = req.params;
    const { search, page = 1, limit = 20 } = req.query;
    const pageNum = parseInt(page);
    ...
    console.error('Error fetching music albums by section:', error);
    res.status(500).json({ error: 'Failed to fetch music albums by section' });
  }
});

// Music Albums - By Artist
router.get('/albums/artist/:artistRatingKey', async (req, res) => {
  try {
    ...
    console.error('Error fetching music albums by artist:', error);
    res.status(500).json({ error: 'Failed to fetch music albums by artist' });
  }
});

// Music Albums - Single Album
router.get('/albums/:ratingKey', async (req, res) => {
  try {
    const { ratingKey } = req.params;
...
router.post('/albums/:ratingKey/extract-file-metadata', async (req, res) => {
  try {
    const { ratingKey } = req.params;
    ...
    res.status(500).json({ error: 'Failed to extract file metadata' });
  }
});
...
router.get('/tracks/section/:sectionKey', async (req, res) => {
  try {
    const { sectionKey } = req.params;
    const { search, page = 1, limit = 20 } = req.query;
    const pageNum = parseInt(page);
    ...
    console.error('Error fetching music tracks by section:', error);
    res.status(500).json({ error: 'Failed to fetch music tracks by section' });
  }
});

// Get tracks by album
router.get('/tracks/album/:albumRatingKey', async (req, res) => {
  try {
...
router.get('/tracks/artist/:artistRatingKey', async (req, res) => {
  try {
    const { artistRatingKey } = req.params;
...
router.get('/stream/:ratingKey', async (req, res) => {
  try {
    const { ratingKey } = req.params;
...
router.get('/debug/:ratingKey', async (req, res) => {
  try {
    const { ratingKey } = req.params;
    const settings = await prisma.settings.findFirst();
