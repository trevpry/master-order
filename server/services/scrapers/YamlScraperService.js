...
class YamlScraperService extends BaseScraperService {
  constructor(yamlFilePath) {
    ...
    // Extract URL patterns from performerByURL configuration
    this.performerUrlPatterns = [];
    if (this.config.performerByURL) {
      this.config.performerByURL.forEach(scraper => {
        if (scraper.url) {
          this.performerUrlPatterns.push(...scraper.url);
        }
      });
    }
  ...
  canHandle(url) {
    ...
    if (matchesMovie) return true;

    // Check if URL matches any of the performer URL patterns
    const matchesPerformer = this.performerUrlPatterns.some(pattern => {
      const normalizedUrl = url.toLowerCase().replace(/^https?:\/\/(www\.)?/, '');
      const normalizedPattern = pattern.toLowerCase().replace(/^https?:\/\/(www\.)?/, '');
  ...
  xpathToJquery(xpath) {
    ...
    // Handle bare attribute presence selectors like [@href]
  ...
  parseDate(dateString, formatString) {
    ...
    try {
      ...
      const monthNameRegex = /^\s*(\d{1,2})\s+(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+(\d{2}|\d{4})\s*$/i;
      const monthNameMatch = rawDate.match(monthNameRegex);
      if (monthNameMatch) {
        const day = monthNameMatch[1].padStart(2, '0');
        const monthKey = monthNameMatch[2].charAt(0).toUpperCase() + monthNameMatch[2].slice(1).toLowerCase();
        const month = monthMap[monthKey];
        const yearPart = monthNameMatch[3];
        const year = yearPart.length === 2 ? `20${yearPart}` : yearPart;

        if (!month) {
          console.warn(`   ⚠️ Unsupported month name in date "${dateString}"`);
          return dateString;
        }
      ...
      // Build regex from Go format string for numeric dates
      ...
      const month = String(parts.month || '00').padStart(2, '0');
      const day = String(parts.day || '00').padStart(2, '0');
  ...
  /**
   * Resolve a dotted path (supports numeric object keys) against a plain object
   */
  ...
  /**
   * Fetch and cache the bearer/instance token some JSON APIs require.
   * The token is scraped out of a normal site page with a regex.
   */
  async _resolveJsonSearchToken(jsonSearch) {
    if (!jsonSearch.tokenUrl || !jsonSearch.tokenRegex) return null;
    if (this._jsonSearchToken) return this._jsonSearchToken;

    const response = await fetch(jsonSearch.tokenUrl, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' }
    });
    const html = await response.text();
    const match = html.match(new RegExp(jsonSearch.tokenRegex));

    if (!match) {
      throw new Error(`Could not extract API token from ${jsonSearch.tokenUrl}`);
    }

    this._jsonSearchToken = match[0];
    return this._jsonSearchToken;
  }

  /**
   * Query a JSON search API described by the sceneByFragment `jsonSearch` config
   * and normalize the records into the same shape the HTML search paths return.
   */
  async _jsonSearchScenes(jsonSearch, searchUrlPattern, query) {
    const token = await this._resolveJsonSearchToken(jsonSearch);
    const url = searchUrlPattern.replace('{query}', encodeURIComponent(query));

    const headers = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)', ...(jsonSearch.headers || {}) };
    if (token && jsonSearch.tokenHeader) {
      headers[jsonSearch.tokenHeader] = token;
    }

    console.log(`   🌐 JSON search: ${url}`);
    const response = await fetch(url, { headers });

    if (!response.ok) {
      throw new Error(`JSON search failed with status ${response.status}`);
    }

    const payload = await response.json();
    const records = this._getPath(payload, jsonSearch.resultsPath || 'result') || [];
    const fields = jsonSearch.fields || {};

    return records.map(record => {
      const id = this._getPath(record, fields.id || 'id');
      const title = this._getPath(record, fields.title || 'title') || null;
      const rawDate = this._getPath(record, fields.date || 'date');
      const slug = String(title || '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '');

      const performerRecords = this._getPath(record, jsonSearch.performersPath || 'performers') || [];
      const tagRecords = this._getPath(record, jsonSearch.tagsPath || 'tags') || [];

      return {
        url: (jsonSearch.urlTemplate || '{id}').replace('{id}', id).replace('{slug}', slug),
        title,
        date: rawDate ? String(rawDate).substring(0, 10) : null,
        coverImage: this._getPath(record, jsonSearch.imagePath) || null,
        studio: this.siteName,
        performers: performerRecords
          .map(p => (typeof p === 'string'
            ? { name: p, id: null }
            : {
                name: this._getPath(p, jsonSearch.performerNameField || 'name'),
                id: this._getPath(p, jsonSearch.performerIdField || 'id')
              }))
          .filter(p => p.name),
        tags: tagRecords
          .map(t => (typeof t === 'string' ? t : this._getPath(t, jsonSearch.tagNameField || 'name')))
          .filter(Boolean)
      };
    });
  }
  ...
  /**
   * Locate a performer's page URL by name, for sites where we only know the name
   * (e.g. the scraper was matched through a linked studio rather than a URL).
   * Tries the JSON search API first, then the HTML performer search page.
   */
  async searchPerformerUrl(name, aliases = []) {
    const searchConfig = this.config.sceneByFragment?.[0];
    if (!searchConfig) return null;

    const namesToTry = [name, ...aliases].filter(Boolean);
    const jsonSearch = searchConfig.jsonSearch;

    if (jsonSearch?.performerUrlTemplate && jsonSearch.performerSearchUrl) {
      for (const candidate of namesToTry) {
        const candidateLower = candidate.toLowerCase();
        try {
          const scenes = await this._jsonSearchScenes(jsonSearch, jsonSearch.performerSearchUrl, candidate);
          const actor = scenes
            .flatMap(scene => scene.performers)
            .find(p => p.id && p.name.toLowerCase() === candidateLower);

          if (actor) {
            const slug = actor.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
            const url = jsonSearch.performerUrlTemplate.replace('{id}', actor.id).replace('{slug}', slug);
            console.log(`   ✓ [${this.siteName}] Resolved performer "${candidate}" to ${url}`);
            return url;
          }
        } catch (error) {
          console.warn(`   ⚠️ [${this.siteName}] JSON performer lookup failed for "${candidate}": ${error.message}`);
        }
      }
    }
    ...
    if (!searchConfig.performerSearchUrl || !performerSearchConfig?.performer) return null;
    ...
    for (const candidate of namesToTry) {
      const candidateLower = candidate.toLowerCase();
      const searchUrl = searchConfig.performerSearchUrl.replace(
        '{performer}',
        candidate.toLowerCase().replace(/\s+/g, spacesConvertTo)
      );

      try {
        const $ = await this.fetchHtml(searchUrl, this.renderJavaScript);
        const elements = this.extractArrayElements($, performerConfig.Name);

        for (let i = 0; i < elements.length; i++) {
          const resultName = this.extractValueAtIndex($, performerConfig.Name, i);
          if (resultName && resultName.toLowerCase().trim() === candidateLower) {
            const url = this.extractValueAtIndex($, performerConfig.URL, i);
            if (url) {
              console.log(`   ✓ [${this.siteName}] Resolved performer "${candidate}" to ${url}`);
              return url;
            }
        ...
        console.warn(`   ⚠️ [${this.siteName}] HTML performer lookup failed for "${candidate}": ${error.message}`);
      }
    }
  ...
  async searchScenes(performers) {
    ...
    // JSON API search takes precedence when configured
    const jsonSearch = searchConfig.jsonSearch;
    if (jsonSearch?.performerSearchUrl) {
      ...
      for (const performer of performers) {
        ...
        if (performer.alias) {
          namesToTry.push(...performer.alias.split(',').map(a => a.trim()).filter(Boolean));
        }

        const matchedUrls = new Set();
        for (const name of namesToTry) {
          const results = await this._jsonSearchScenes(jsonSearch, jsonSearch.performerSearchUrl, name);
          const nameLower = name.toLowerCase();

          results
            .filter(scene => scene.performers.some(p => p.name.toLowerCase() === nameLower))
            .forEach(scene => {
              scenesByUrl.set(scene.url, scene);
              matchedUrls.add(scene.url);
            });
        }

        matchedUrls.forEach(url => matchCounts.set(url, (matchCounts.get(url) || 0) + 1));
      }

      // Keep only scenes that matched every requested performer
      const matching = [...scenesByUrl.values()].filter(scene => matchCounts.get(scene.url) === performers.length);

      console.log(`   ✅ Found ${matching.length} scene(s) with ALL ${performers.length} performer(s)`);

      return matching.map(scene => ({
        url: scene.url,
        title: scene.title,
        date: scene.date,
  ...
  async searchByTitle(title, studioUrl = null) {
    ...
    // JSON API search takes precedence when configured
    const jsonSearch = searchConfig.jsonSearch;
    if (jsonSearch?.titleSearchUrl) {
      const results = await this._jsonSearchScenes(jsonSearch, jsonSearch.titleSearchUrl, title);
      console.log(`   ✅ Found ${results.length} scene(s) matching title: "${title}"`);
    ...
    try {
      ...
      while (pageCount < maxPages) {
        ...
        const countBeforePage = allScenes.length;
        for (let i = 0; i < titleElements.length; i++) {
          const scene = {
            title: null,
            url: null,
          ...
          if (scene.title && scene.url && !seenSceneUrls.has(scene.url)) {
            seenSceneUrls.add(scene.url);
            allScenes.push(scene);
          }
        }

        // Sites that ignore the page parameter re-serve page 1 forever
        if (allScenes.length === countBeforePage) {
          console.log(`   ✓ Page ${pageCount} added no new scenes, stopping pagination`);
          break;
        }
      }
  ...
  /**
   * Scrape performer metadata from a performer URL
   * @param {string} url - Performer URL to scrape
   * @returns {Promise<Object>} - Scraped performer metadata
   */
  async scrapePerformer(url) {
    console.log(`👤 [${this.siteName}] Scraping performer: ${url}`);

    const performerConfig = Array.isArray(this.config.performerByURL)
      ? this.config.performerByURL[0]
      : this.config.performerByURL;

    if (!performerConfig) {
      throw new Error(`${this.siteName} does not support performer scraping`);
    }

    try {
      if (this.isScriptBased && performerConfig.action === 'script' && performerConfig.script) {
        const result = await this._executeScript(performerConfig.script, 'performer-by-url', { url });
        const normalized = this._normalizeScriptResult(result);
        return this.formatResult(normalized);
      }

      const $ = await this.fetchHtml(url, this.renderJavaScript);
      const scraperName = performerConfig.scraper;
      const scraperConfig = this.config.xPathScrapers[scraperName];

      if (!scraperConfig || !scraperConfig.performer) {
        throw new Error(`Performer scraper configuration not found: ${scraperName}`);
      }

      const performer = scraperConfig.performer;
      const metadata = {
        url,
        title: null,
        name: null,
      ...
      if (performer.Name) {
        metadata.name = this.extractValue($, performer.Name);
      }
      if (performer.Gender) {
        metadata.gender = this.extractValue($, performer.Gender);
      }
      if (performer.Details) {
        metadata.details = this.extractValue($, performer.Details);
      }
      if (performer.Image) {
        metadata.image = this.extractValue($, performer.Image);
      }
      if (performer.Ethnicity) {
        metadata.ethnicity = this.extractValue($, performer.Ethnicity);
      }
      if (performer.EyeColor) {
        metadata.eyeColor = this.extractValue($, performer.EyeColor);
      }
      if (performer.HairColor) {
        metadata.hairColor = this.extractValue($, performer.HairColor);
      }
      if (performer.Height) {
        metadata.height = this.extractValue($, performer.Height);
      }
      if (performer.PenisLength) {
        metadata.penisLength = this.extractValue($, performer.PenisLength);
      }
      if (performer.Circumcised) {
        metadata.circumcised = this.extractValue($, performer.Circumcised);
      }
      if (performer.Weight) {
        metadata.weight = this.extractValue($, performer.Weight);
      }
      if (performer.Measurements) {
        metadata.measurements = this.extractValue($, performer.Measurements);
      }
      if (performer.Country) {
        metadata.country = this.extractValue($, performer.Country);
      }
      if (performer.Birthdate) {
        metadata.birthdate = this.extractValue($, performer.Birthdate);
      }
      if (performer.Tattoos) {
        metadata.tattoos = this.extractValue($, performer.Tattoos);
      }
      if (performer.Piercings) {
        metadata.piercings = this.extractValue($, performer.Piercings);
      }
      if (performer.CareerLength) {
        metadata.careerLength = this.extractValue($, performer.CareerLength);
      }
      if (performer.Aliases) {
        metadata.aliases = this.extractArray($, performer.Aliases);
      }
      if (performer.Tags?.Name) {
        metadata.tags = this.extractArray($, performer.Tags.Name);
      }
      ...
      // formatResult() is scene-shaped and would drop every performer field.
      // Keys are snake_case to match the Stash performer schema and review modals.
      return {
        success: true,
        ...
        scraped: {
          ...
          image: metadata.image || metadata.coverImage || null,
          birthdate: metadata.birthdate || null,
          death_date: metadata.deathDate || null,
          ...
          penis_length: metadata.penisLength,
          circumcised: metadata.circumcised || null,
          tattoos: metadata.tattoos || null,
          piercings: metadata.piercings || null,
          career_length: metadata.careerLength || null,
          aliases: metadata.aliases || [],
          tags: metadata.tags || []
        }
      };
    } catch (error) {
      console.error(`❌ [${this.siteName}] Error scraping performer:`, error);
      throw new Error(`Failed to scrape ${this.siteName} performer: ${error.message}`);
    }
  }
}
